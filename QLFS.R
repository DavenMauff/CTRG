...
qlfsDataSpread <- qlfsDataSpread %>%
  mutate(`Average Total/Year` = round((rowSums(qlfsDataSpread[2:40]))/8, digits = 0))

#CALULATING AVERAGE HIGHER EDUCATION TOTAL PER YEAR
qlfsDataSpread <- qlfsDataSpread %>%
  mutate(`Average Higher Education/Year` = round((rowSums(qlfsDataSpread[higherEducation])/8), digits = 0))
...
qlfsDataFOS2 <- data.frame(with(qlfsDataFOS2, table(field, status, year)))
...
qlfsDataFOS2$field <- as.character(qlfsDataFOS2$field)


relative_unemployment <- as.data.frame(apply(qlfsDataFOS2[,2:6], 2, function(x) tapply(x, qlfsDataFOS2$field, sum)))
